                    ...
                    >
                      <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-1">
                          <svg v-for="i in getSkillLevelFromObject(skill)" :key="i" width="10" height="10" viewBox="0 0 24 24" fill="currentColor" class="text-yellow-400">
                            <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>
                          </svg>
                          <svg v-for="i in (5 - getSkillLevelFromObject(skill))" :key="`empty-${i}`" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" class="text-gray-300">
                            <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/>
                          </svg>
                        </div>
                      </div>
...
const getSkillLevelFromObject = (skill) => {
  // If skill is an object with level, return it directly
  if (typeof skill === 'object' && skill.level) {
    return skill.level
  }
  
  // Otherwise fall back to the old logic
  return getSkillLevel(getSkillName(skill))
}
